---
layout: home
title: Sudhanshu Pandey
---

# Sudhanshu Pandey

🛰️ Scientist at **NASA's Jet Propulsion Laboratory**.
Methane Modeling Lead for the **U.S. Greenhouse Gas Center**.
I quantify greenhouse-gas emissions from space — from single leaking wells
to the global carbon cycle.

## About

My research stitches satellite remote sensing to atmospheric modeling. A few
threads I keep returning to:

- **Catching methane leaks from orbit.** Our
  [2019 PNAS paper](https://doi.org/10.1073/pnas.1908712116) was the
  first satellite detection of an unreported extreme methane leak — a
  natural-gas well blowout that turned out to be one of the largest U.S.
  point-source emissions on record. It helped kick off a wave of operational
  satellite-based plume monitoring.
- **Taking Earth's carbon pulse from space.** I developed satellite-derived
  whole-atmosphere CO₂ growth-rate methods now part of the Global Carbon
  Project workflow
  ([AGU Adv. 2024](https://doi.org/10.1029/2023AV001145);
  [Nat. Commun. 2025](https://doi.org/10.1038/s41467-025-61588-2);
  [AGU Adv. 2025](https://doi.org/10.1029/2025AV002085)).
- **AI for atmospheric science.** Building CNNs for emission-source detection
  and quantification, ML emulators for atmospheric tracer transport, and
  LLM-based agentic systems to automate and accelerate climate and
  atmospheric research.

I did my **PhD in physics at Utrecht University** (2017) and an integrated
BS-MS in Earth Sciences at IISER Kolkata. Before JPL I spent six years at
SRON, the Netherlands Institute for Space Research.

## What I'm working on now

- ML/physics hybrids for global methane and CO₂ inverse modeling.
- Multi-instrument satellite methane plume detection (Sentinel-2 / 3 / 5P,
  PRISMA, EMIT and friends).
- LLM-based autonomous research agents for atmospheric inversions and
  scientific discovery.
- Side projects — like the [waterfall ML benchmark](/2026/06/08/yosemite-waterfall-ml-vs-4dvar.html) below.

## Selected publications

Full list in my [CV](https://science.jpl.nasa.gov/documents/1475/CV_Sudhanshu_Pandey_20260415.pdf)
or [Google Scholar](https://scholar.google.com/citations?user=efFF_TEAAAAJ&hl=en).

- **Pandey, S.** (2025). Taking Earth's carbon pulse from space.
  *AGU Advances.*
  [doi](https://doi.org/10.1029/2025AV002085)
- **Pandey, S.**, et al. (2025). Reduction in Earth's carbon budget imbalance.
  *Nature Communications* 16, 6818.
  [doi](https://doi.org/10.1038/s41467-025-61588-2)
- **Pandey, S.**, et al. (2025). Relating multi-scale plume detection and area
  estimates of methane emissions. *Environmental Science & Technology* 59,
  7931–7947. [doi](https://doi.org/10.1021/acs.est.4c07415)
- **Pandey, S.**, et al. (2024). Toward low-latency estimation of atmospheric
  CO₂ growth rates using satellite observations. *AGU Advances* 5,
  e2023AV001145. [doi](https://doi.org/10.1029/2023AV001145)
- **Pandey, S.**, et al. (2023). Daily detection and quantification of methane
  leaks using Sentinel-3.
  *Remote Sensing of Environment* 296, 113716.
  [doi](https://doi.org/10.1016/j.rse.2023.113716)
- **Pandey, S.**, et al. (2022). Order-of-magnitude wall-time improvement of
  variational methane inversions by physical parallelization.
  *Geoscientific Model Development* 15, 4555–4567.
  [doi](https://doi.org/10.5194/gmd-15-4555-2022)
- **Pandey, S.**, et al. (2019). Satellite observations reveal extreme methane
  leakage from a natural gas well blowout.
  *PNAS* 116(52), 26376–26381.
  [doi](https://doi.org/10.1073/pnas.1908712116)
- **Pandey, S.**, et al. (2017). Enhanced methane emissions from tropical
  wetlands during the 2011 La Niña.
  *Scientific Reports* 7, 45759.
  [doi](https://doi.org/10.1038/srep45759)

## A few awards

- NASA Group Achievement Award — Greenhouse Gas Center Team (2026)
- NASA ROSES Early Career Investigator Program in Earth Science (2023)
- INSPIRE Fellowship, Indian Department of Science & Technology (2008)

## Service

Reviewer for *Nature*, *Science Advances*, *Nature Climate Change*, *AMT*,
*ACP*, *GRL*, and others. Review Editor at *Frontiers*. Proposal reviewer
for NASA and NOAA. Poster judge at EGU and AGU. I've mentored students and
interns at JPL and SRON drawn from Caltech, USC, Cal Poly Pomona, Harvard,
TU Delft, and Utrecht.

### Find me elsewhere

- 🌐 [JPL profile](https://science.jpl.nasa.gov/people/pandeysu/)
- 🎓 [Google Scholar](https://scholar.google.com/citations?user=efFF_TEAAAAJ&hl=en)
- 💻 [GitHub: @sudshu](https://github.com/sudshu)
- 📄 [CV (PDF)](https://science.jpl.nasa.gov/documents/1475/CV_Sudhanshu_Pandey_20260415.pdf)

*Views and weekend experiments here are my own and don't represent NASA or
JPL.*

---

## Recent posts
